...
- `telemetry(event)` — fires once per `(table, op)` group of each transaction the runtime applies, with `{ schema, table, op, count, committedAt, appliedAt, commitLsn }`; `count` is the number of row events in the group; both stamps are epoch microseconds, so `appliedAt - committedAt` is the sync latency in microseconds (`committedAt` is the database host's commit clock, `appliedAt` this process's clock taken once the whole commit batch is visible to collections — cross-host skew can make individual deltas negative); runs synchronously on the replication loop, so keep it cheap; a throwing callback is logged and never disrupts replication; a pk-changing UPDATE applies as delete-then-insert and counts into both groups; rows recovered by a post-reconnect rebaseline arrive outside the WAL stream and emit nothing, so counts show gaps around reconnects.
