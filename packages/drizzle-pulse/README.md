...
- `telemetry(event)` — fires once per row event the runtime applies, with `{ schema, table, op, committedAt, appliedAt }`; both stamps are epoch microseconds, so `appliedAt - committedAt` is the sync latency in microseconds (`committedAt` is the database host's commit clock, `appliedAt` this process's apply clock — cross-host skew can make individual deltas negative); runs synchronously on the replication loop, so keep it cheap; a throwing callback is logged and never disrupts replication; a pk-changing UPDATE applies as delete-then-insert and reports both.
